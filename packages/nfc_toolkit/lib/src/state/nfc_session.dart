...
class NfcSessionAction {
  ...
  factory NfcSessionAction.success({String? message}) {
    return NfcSessionAction._(isSuccess: true, message: message);
  }
  ...
  factory NfcSessionAction.error({required String message}) {
    return NfcSessionAction._(isSuccess: false, message: message);
  }

  /// The UI processed the tag but no specific action is required.
  /// This closes the session with a default message if possible.
  factory NfcSessionAction.none() {
    return const NfcSessionAction._(isSuccess: true, isNone: true);
  }
}
